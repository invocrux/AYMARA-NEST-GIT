...
export class AymaraService {
  ...
  // Ya no usamos filtrado de preguntas, confiamos completamente en el modelo
  ...
  constructor(private readonly configService: ConfigService) {
    ...
    // Mensaje del sistema para AYMARA sin restricciones temáticas
    this.SYSTEM_MESSAGE = `
Eres AYMARA, una inteligencia artificial versátil creada para asistir en cualquier tema que el usuario necesite.
Tu objetivo es proporcionar respuestas útiles, precisas y adaptadas a las necesidades del usuario.

Reglas y estilo de respuesta: 
1. Responde a cualquier tipo de consulta que el usuario te plantee.
2. Prioriza la precisión sobre la extensión: responde de forma breve pero completa. 
3. Usa ejemplos prácticos cuando sea apropiado. 
4. Mantén un tono profesional y neutro. 
5. Si no tienes datos suficientes para responder con seguridad, indícalo claramente. 
...
- Referencias relacionadas (cuando sea relevante). 
...
`;
  ...
  async procesarConsulta(
    ...
    const { pregunta } = createConsultaDto;

    // Registrar la consulta en logs
    this.logger.log(`Nueva consulta recibida: ${pregunta}`);
    if (pregunta) {
      this.logger.log(
        `Contexto adicional proporcionado: ${pregunta.substring(0, 100)}${pregunta.length > 100 ? "..." : ""}`
      );
    }

    // Ya no validamos si la pregunta está dentro del alcance
    this.logger.log(`Procesando consulta: ${pregunta}`);
